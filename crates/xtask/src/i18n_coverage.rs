...
fn starts_ignored_block(trimmed: &str) -> bool {
    trimmed.starts_with("#[cfg(test)]")
        ...
        || trimmed.starts_with("pub fn init(app: &mut AppContext)")
...
fn is_ignored_literal(literal: &str) -> bool {
    ...
    if literal.starts_with("settings-")
        ...
        || literal.starts_with("comment_list_view:")
        || literal.starts_with("diffset against ")
        ...
        || literal.starts_with("Attempted to ")
        || literal.starts_with("CodeReviewView ")
        || literal.starts_with("Code Review:")
    ...
    if matches!(
        ...
        "ESC"
            ...
            | "Code review is not available on wasm"
            | "get_match_character_bounds should not run on wasm"
            | "horizontally_scroll_to_match should not run on wasm"
            | "state was verified as Loaded"
            | "Empty diff hunk"
            | "Invalid header: {err}"
            | "Unexpected hunk header at line index {line_index}"
            | "Target line {target_line} not found in hunk"
