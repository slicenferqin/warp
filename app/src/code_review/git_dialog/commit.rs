...
pub(super) fn new_state(
    ...
    let (push_label, push_icon) = if has_upstream {
        (
            warp_i18n::tr("app-code-review-git-commit-and-push"),
            Icon::ArrowUp,
        )
    } else {
        (
            warp_i18n::tr("app-code-review-git-commit-and-publish"),
            Icon::UploadCloud,
        )
    };
    ...
    let initial_placeholder = if ai_autogen_enabled {
        warp_i18n::tr("app-code-review-git-generating-commit-message")
    } else {
        warp_i18n::tr("app-code-review-git-type-commit-message")
    };
    let message_editor = ctx.add_typed_action_view(move |ctx| {
        ...
        editor.set_placeholder_text(initial_placeholder.clone(), ctx);
        editor
    });
    ...
    let commit_and_create_pr_button = if allow_create_pr {
        Some(ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(
                warp_i18n::tr("app-code-review-git-commit-and-create-pr"),
                SecondaryTheme,
            )
            .with_size(ButtonSize::XSmall)
            .with_height(32.)
...
pub(super) fn confirm_tooltip(state: &CommitState, app: &AppContext) -> Option<String> {
    if !state.file_changes.is_empty() && commit_message(state, app).is_none() {
        Some(warp_i18n::tr("app-code-review-git-enter-commit-message"))
    } else {
        None
    }
}
...
pub(super) fn start_confirm(me: &mut GitDialog, ctx: &mut ViewContext<GitDialog>) {
    ...
    me.set_loading(warp_i18n::tr("app-code-review-git-committing"), ctx);
        ...
        move |_me, result, ctx| {
            ...
            match result {
                Ok(CommitOutcome::Committed) => {
                    show_toast(warp_i18n::tr("app-code-review-git-commit-success"), ctx);
                }
                Ok(CommitOutcome::Pushed) => {
                    show_toast(
                        warp_i18n::tr("app-code-review-git-commit-push-success"),
                        ctx,
                    );
                }
...
fn render_changes_section(state: &CommitState, appearance: &Appearance) -> Box<dyn Element> {
    ...
    let include_label = Text::new(
        warp_i18n::tr("app-code-review-git-include-unstaged"),
        appearance.ui_font_family(),
        appearance.ui_font_size(),
    )
...
fn render_message_editor(
    ...
    let label = Text::new(
        warp_i18n::tr("app-code-review-git-commit-message"),
        appearance.ui_font_family(),
        appearance.ui_font_size(),
    )
    .with_color(
