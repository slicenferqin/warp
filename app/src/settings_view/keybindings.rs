...
pub const SEARCH_PLACEHOLDER: &str = "settings-keybindings-search-placeholder";
const SHORTCUT_CONFLICT_WARNING_TEXT_KEY: &str = "settings-keybindings-shortcut-conflict";
const KEYBINDINGS_PAGE_SHORTCUT: &str = "workspace:toggle_keybindings_page";
const RESET_BUTTON_TEXT_KEY: &str = "common-default";
const CANCEL_BUTTON_TEXT_KEY: &str = "common-cancel";
const CLEAR_BUTTON_TEXT_KEY: &str = "settings-keybindings-clear";
const SAVE_BUTTON_TEXT_KEY: &str = "common-save";
...
impl KeybindingRow {
    ...
    fn render_clicked(
        ...
        let conflict_warning = if has_conflicting_binding {
            render_text(
                warp_i18n::tr(SHORTCUT_CONFLICT_WARNING_TEXT_KEY),
        ...
        let press_new_shortcut_text = render_text(
            warp_i18n::tr("settings-keybindings-press-new-shortcut"),
            None,
            appearance,
        );
    ...
    fn get_edit_button_row(&self, appearance: &Appearance, index: usize) -> Box<dyn Element> {
        ...
        if self.binding.trigger.is_some() {
            let clear = Hoverable::new(
                ...
                |state| {
                    render_button(
                        warp_i18n::tr(CLEAR_BUTTON_TEXT_KEY),
                        appearance,
        ...
        let clear = Container::new(
            Hoverable::new(
                ...
                |state| {
                    render_button(
                        warp_i18n::tr(RESET_BUTTON_TEXT_KEY),
                        appearance,
                        self.get_button_text_color(appearance, state),
                    )
                },
            ...
            Hoverable::new(
                ...
                |state| {
                    ...
                    if index == 0 {
                        SavePosition::new(
                            render_button(
                                warp_i18n::tr(CANCEL_BUTTON_TEXT_KEY),
                                appearance,
                                cancel_button_color,
                            ),
        ...
        let save = Container::new(
            Hoverable::new(self.mouse_state_handles.save_mouse_state.clone(), |state| {
                render_button(
                    warp_i18n::tr(SAVE_BUTTON_TEXT_KEY),
                    appearance,
                    self.get_button_text_color(appearance, state),
                )
            })
...
impl KeybindingsView {
    pub fn new(ctx: &mut ViewContext<KeybindingsView>) -> Self {
        ...
        search_editor.update(ctx, |editor, ctx| {
            ...
            editor.set_placeholder_text(warp_i18n::tr(SEARCH_PLACEHOLDER), ctx);
        });
...
fn render_text(
    ...
    let mut text = appearance.ui_builder().wrappable_text(text.into(), true);
...
impl KeybindingsWidget {
    fn render_description(
        ...
        let mut description = Flex::column().with_child(render_text(
            warp_i18n::tr("settings-keybindings-description"),
            Some(UiComponentStyles {
                ...
                Wrap::row()
                    .with_child(
                        Container::new(render_text(
                            warp_i18n::tr("settings-keybindings-use"),
                            Some(UiComponentStyles {
                                font_size: Some(font_size),
                                font_color: Some(
                                    appearance
                    ...
                    .with_child(
                        Container::new(render_text(
                            warp_i18n::tr("settings-keybindings-reference-side-pane"),
                            Some(UiComponentStyles {
                                font_size: Some(font_size),
                                font_color: Some(
                                    appearance
    ...
    fn render(
        ...
        let local_only_icon_state = if *CloudPreferencesSettings::as_ref(app).settings_sync_enabled
        {
            Some(LocalOnlyIconState::Visible {
                ...
                custom_tooltip: Some(warp_i18n::tr("settings-keybindings-not-synced")),
            })
        } else {
            None
        };
        ...
        let subheader = render_sub_header(
            appearance,
            warp_i18n::tr("settings-keybindings-configure"),
        ...
        Flex::column()
            ...
            .with_child(render_columns(
                Container::new(render_text(
                    warp_i18n::tr("settings-keybindings-command"),
                    Some(UiComponentStyles {
