...
impl AgentAssistedEnvironmentModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let add_repo_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(warp_i18n::tr("settings-agent-env-add-repo"), SecondaryTheme)
        ...
        let cancel_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(warp_i18n::tr("common-cancel"), SecondaryTheme).on_click(|ctx| {
        ...
        let create_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(
                warp_i18n::tr("settings-agent-env-create-environment"),
                PrimaryTheme,
            )
            .on_click(|ctx| {
    ...
    fn render_selected_section(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        col.add_child(self.render_section_title(
            warp_i18n::tr("settings-agent-env-selected-repos"),
            appearance,
        ));
        ...
        if self.selected_repo_paths.is_empty() {
            col.add_child(
                Text::new(
                    warp_i18n::tr("settings-agent-env-no-repos-selected"),
                    appearance.ui_font_family(),
    ...
    fn render_available_section(&self, appearance: &Appearance) -> Box<dyn Element> {
            ...
            .with_child(
                Expanded::new(
                    ...
                    self.render_section_title(
                        warp_i18n::tr("settings-agent-env-available-indexed-repos"),
                        appearance,
                    ),
                )
                .finish(),
        ...
        if self.available_repos.is_empty() {
            let text = if cfg!(all(feature = "local_fs", not(target_family = "wasm"))) {
                if self.available_repos_loading {
                    warp_i18n::tr("settings-agent-env-loading-indexed-repos")
                } else {
                    warp_i18n::tr("settings-agent-env-no-indexed-repos")
                }
            } else {
                warp_i18n::tr("settings-agent-env-local-selection-unavailable")
            };
        ...
        if !has_any_available {
            col.add_child(
                Text::new(
                    warp_i18n::tr("settings-agent-env-all-repos-selected"),
                    appearance.ui_font_family(),
                    appearance.ui_font_size() * 0.95,
                )
                .with_color(theme.nonactive_ui_text_color().into())
    ...
    fn show_not_a_repo_toast(&self, selected_path: &Path, ctx: &mut ViewContext<Self>) {
        ...
        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            let toast = DismissibleToast::error(warp_i18n::tr_with_args(
                "settings-agent-env-not-git-repo",
                &[("path", path.as_str())],
            ))
            ...
            move |paths_result, ctx| {
                let result = paths_result.and_then(|paths| {
                    paths.into_iter().next().map(PathBuf::from).ok_or_else(|| {
                        FilePickerError::DialogFailed(warp_i18n::tr(
                            "settings-agent-env-no-directory-selected",
                        ))
                    })
                });
    ...
    fn render_dialog(&self, appearance: &Appearance, app: &AppContext) -> Box<dyn Element> {
        let description = if FeatureFlag::FullSourceCodeEmbedding.is_enabled() {
            warp_i18n::tr("settings-agent-env-description-indexed")
        } else {
            warp_i18n::tr("settings-agent-env-description")
        };
        ...
        let dialog = Dialog::new(
            warp_i18n::tr("settings-agent-env-title"),
            Some(description),
