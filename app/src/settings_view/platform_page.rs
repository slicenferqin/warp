...
impl PlatformPageView {
    ...
    pub fn new(ctx: &mut ViewContext<PlatformPageView>) -> Self {
        ...
        let create_api_key_modal_view = ctx.add_typed_action_view(|ctx| {
            Modal::new(
                Some(warp_i18n::tr("settings-platform-new-api-key")),
                create_api_key_body,
                ctx,
            )
    ...
    fn show_create_api_key_modal(&mut self, ctx: &mut ViewContext<Self>) {
        // Ensure header reads the form title when opening the form.
        self.create_api_key_modal_state
            .set_title(Some(warp_i18n::tr("settings-platform-new-api-key")), ctx);
    ...
    fn handle_create_api_key_modal_event(
        ...
        match event {
            ...
            CreateApiKeyModalEvent::Created { api_key } => {
                ...
                self.create_api_key_modal_state
                    .set_title(Some(warp_i18n::tr("settings-platform-save-your-key")), ctx);
    ...
    fn ensure_expire_button_for_key(&mut self, ctx: &mut ViewContext<Self>, uid: ApiKeyUid) {
        ...
        ctx.subscribe_to_view(&handle, |me, _emitter, event, ctx| match event {
            ExpireApiKeyButtonEvent::ExpireApiKeySucceeded { uid } => {
                ...
                let window_id = ctx.window_id();
                crate::ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = crate::view_components::DismissibleToast::success(warp_i18n::tr(
...
impl PlatformPageWidget {
    fn render_description_with_link(
        ...
        let text = vec![
            FormattedTextFragment::plain_text(format!(
                "{}\n{} ",
                warp_i18n::tr("settings-platform-description"),
                warp_i18n::tr("settings-platform-description-docs-prefix")
            )),
            FormattedTextFragment::hyperlink(
                warp_i18n::tr("settings-platform-documentation"),
                API_KEY_DOCS_URL,
            ),
        ];
    ...
    fn render_api_keys_section(
        ...
        col.add_child(
            Flex::row()
                ...
                .with_child(
                    Text::new_inline(
                        warp_i18n::tr("settings-platform-title"),
                        appearance.ui_font_family(),
                        16.,
                    )
                ...
                .with_child(
                    ui_builder
                        ...
                        .with_text_label(warp_i18n::tr("settings-platform-create-api-key"))
                        .build()
                        .on_click(|ctx, _, _| {
    ...
    fn render_api_keys_header(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        header_row.add_child(
            Expanded::new(
                1.,
                self.render_header_cell(appearance, warp_i18n::tr("settings-platform-name")),
            )
            .finish(),
        );
        header_row.add_child(
            Expanded::new(
                1.,
                self.render_header_cell(appearance, warp_i18n::tr("settings-platform-header-key")),
            )
            .finish(),
        );
            ...
            header_row.add_child(
                Expanded::new(
                    ...
                    self.render_header_cell(
                        appearance,
                        warp_i18n::tr("settings-platform-header-scope"),
                    ),
                )
                .finish(),
            );
        ...
        header_row.add_child(
            Expanded::new(
                ...
                self.render_header_cell(
                    appearance,
                    warp_i18n::tr("settings-platform-header-created"),
                ),
            )
            .finish(),
        );
        header_row.add_child(
            Expanded::new(
                ...
                self.render_header_cell(
                    appearance,
                    warp_i18n::tr("settings-platform-header-last-used"),
                ),
            )
            .finish(),
        );
        header_row.add_child(
            Expanded::new(
                ...
                self.render_header_cell(
                    appearance,
                    warp_i18n::tr("settings-platform-header-expires-at"),
                ),
            )
            .finish(),
        );
        header_row.add_child(
            Expanded::new(0.5, self.render_header_cell(appearance, String::new())).finish(),
        );
    ...
    fn render_header_cell(&self, appearance: &Appearance, label: String) -> Box<dyn Element> {
        Container::new(
    ...
    fn render_api_key_row(
        ...
        let last_used = key
            ...
            .unwrap_or_else(|| warp_i18n::tr("settings-platform-expiration-never"));
        let expires_at = key
            .expires_at
        ...
        if FeatureFlag::TeamApiKeys.is_enabled() {
            let scope_display = match key.scope {
                ApiKeyScope::Personal => warp_i18n::tr("settings-platform-personal"),
                ApiKeyScope::Team => warp_i18n::tr("settings-platform-team"),
            };
            row.add_child(
                Expanded::new(
                    1.,
            ...
            Align::new(
                Flex::column()
                    ...
                    .with_child(
                        Container::new(
                            Text::new(
                                warp_i18n::tr("settings-platform-no-api-keys"),
                                appearance.ui_font_family(),
                                SUBHEADER_FONT_SIZE,
                            )
                            .with_color(appearance.theme().active_ui_text_color().into())
                    ...
                    .with_child(
                        Container::new(
                            Text::new(
                                warp_i18n::tr("settings-platform-zero-state"),
                                appearance.ui_font_family(),
                                CONTENT_FONT_SIZE,
                            )
                            .with_color(appearance.theme().active_ui_text_color().into())
